...
class LessonCRUDTestCase(APITestCase):
    ...
    def test_create_lesson_authenticated_owner(self):
        ...
        if response.status_code == status.HTTP_201_CREATED:
            ...
            self.assertEqual(
                created_lesson.video_url, "https://www.youtube.com/watch?v=newtest"
            )
    ...
    def test_update_lesson_owner(self):
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            self.assertEqual(
                self.lesson.video_url, "https://www.youtube.com/watch?v=updated"
            )
            ...
            self.assertEqual(
                response.data["video_url"], "https://www.youtube.com/watch?v=updated"
            )
            self.assertEqual(response.data["course"], self.course.id)
            self.assertEqual(response.data["owner"], self.owner_user.id)
        elif response.status_code == status.HTTP_403_FORBIDDEN:
